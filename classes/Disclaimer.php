	...
	public static function show(DisclaimerArchiveModel $objDisclaimerArchive, $varAccept)
	{
		...
		switch ($objDisclaimer->source) {
			case 'page':
				...
				break;
			case 'article':
				if (($objArticle = \ArticleModel::findByPk($objDisclaimer->articleId, array('eager' => true))) !== null && ($objPid = $objArticle->getRelated('pid')) !== null) {
					/** @var \PageModel $objPid */
					$strUrl = ampersand($objPid->getFrontendUrl('/articles/' . ((!\Config::get('disableAlias') && $objArticle->alias != '') ? $objArticle->alias : $objArticle->id)));
