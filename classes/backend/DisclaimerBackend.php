...
class DisclaimerBackend extends \Backend
{
	...
	public function checkFallback($varValue, \DataContainer $dc)
	{
		...
		$objPage = \Database::getInstance()->prepare("SELECT id FROM tl_disclaimer WHERE fallback=1 AND pid=? AND id!=?")
	...
	public function listChildren($arrRow)
	{
		...
		return '<div class="tl_content_left">' . ($language ? '[' . $language . '] ': '') . ($arrRow['title'] ?: $arrRow['id']) . ' <span style="color:#b3b3b3; padding-left:3px">[' .
		...
		switch (\Input::get('act')) {
			...
			case 'create':
				if (!strlen(\Input::get('pid')) || !in_array(\Input::get('pid'), $root)) {
					\Controller::log('Not enough permissions to create disclaimer items in disclaimer archive ID "' . \Input::get('pid') . '"', 'tl_disclaimer checkPermission', TL_ERROR);
			...
			case 'toggle':
			case 'feature':
				$objArchive = $objDatabase->prepare("SELECT pid FROM tl_disclaimer WHERE id=?")
					->limit(1)->execute($id);
	...
	public function toggleVisibility($intId, $blnVisible)
	{
		...
		// Update the database
		$objDatabase->prepare("UPDATE tl_disclaimer SET tstamp=" . time() . ", published='" . ($blnVisible ? 1 : '') . "' WHERE id=?")
			->execute($intId);
