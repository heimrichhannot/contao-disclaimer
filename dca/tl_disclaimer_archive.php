...
$GLOBALS['TL_DCA']['tl_disclaimer_archive'] = array(
    ...
    'list'        => array(
        ...
        'operations'        => array(
            ...
            'editheader' => array(
                'label'           => &$GLOBALS['TL_LANG']['tl_disclaimer_archive']['editheader'],
                'href'            => 'act=edit',
                'icon'            => 'header.gif',
                'button_callback' => array('tl_disclaimer_archive', 'editHeader'),
            ),
            'copy'       => array(
            ...
            'toggle'     => array(
                ...
                'button_callback' => array('tl_disclaimer_archive', 'toggleIcon'),
            ),
            'show'       => array(
...
class tl_disclaimer_archive extends \Backend
{
    ...
    public function toggleIcon($row, $href, $label, $title, $icon, $attributes)
    {
        ...
        if (!$objUser->isAdmin && !$objUser->hasAccess('tl_disclaimer_archive::published', 'alexf'))
        {
            return '';
        }
    ...
    public function toggleVisibility($intId, $blnVisible)
    {
        ...
        if (!$objUser->isAdmin && !$objUser->hasAccess('tl_disclaimer_archive::published', 'alexf'))
        {
            \Controller::log('Not enough permissions to publish/unpublish item ID "' . $intId . '"', 'tl_disclaimer_archive toggleVisibility', TL_ERROR);
            \Controller::redirect('contao/main.php?act=error');
        }

        $objVersions = new \Versions('tl_disclaimer_archive', $intId);
        $objVersions->initialize();

        // Trigger the save_callback
        if (is_array($GLOBALS['TL_DCA']['tl_disclaimer_archive']['fields']['published']['save_callback']))
        {
            foreach ($GLOBALS['TL_DCA']['tl_disclaimer_archive']['fields']['published']['save_callback'] as $callback)
            {
        ...
        // Update the database
        $objDatabase->prepare("UPDATE tl_disclaimer_archive SET tstamp=" . time() . ", published='" . ($blnVisible ? 1 : '') . "' WHERE id=?")->execute($intId);

        $objVersions->create();
        \Controller::log(
            'A new version of record "tl_disclaimer_archive.id=' . $intId . '" has been created' . $this->getParentEntries('tl_disclaimer_archive', $intId),
            'tl_disclaimer_archive toggleVisibility()',
            TL_GENERAL
        );
    }
}
